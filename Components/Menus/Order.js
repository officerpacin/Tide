...
module.exports = {
    ...
    execute: async (client, interaction) => {
        if (interaction.values.includes('Regular')) {
            if (interaction.member.roles.cache.has('1377484739245637652')) return await interaction.reply({ content: `Order canceled! You cannot place an order as you have the order blacklist role.`, flags: ['Ephemeral'] });
            ...
            const ViewTermsButton = new ButtonBuilder()
                ...
                .setURL('https://docs.google.com/document/d/169fTAO5k-pWJFuCy1MT_Pa_RO6CabaPxvLlKlOwCuqo/edit?tab=t.0#heading=h.v0xalj9ip4ii')
        ...
        if (interaction.values.includes('Priority')) {
            if (!interaction.member.roles.cache.has('1378237435430834348')) return await interaction.reply({ content: `Only members with the priority role may open priority orders!`, flags: ['Ephemeral'] });
